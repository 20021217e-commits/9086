...
void FA_ReadDisturb(void)
{
	...
	U32 GroupModelLen = (U32)(pCtrl->dWareIdx);
	...
	else
	{
		...
		for (i = 0; i < GroupModel_Stack_Idx; i++)
		{
			...
			{
				PageList[0] += GroupModel_Stack[i] * GroupModel[GroupModel_SP[GroupModel_Stack_Idx]];
			}
			else
			{
				PageList[0] += (WlAddr - GroupModel_Stack_Sum) * GroupModel[GroupModel_SP[GroupModel_Stack_Idx]];
				for (j = 1; j < GroupModel_Len[i]; j++)
				{
					PageList[j] = PageList[0] + GroupModel[GroupModel_SP[GroupModel_Stack_Idx] + j];
				}
			}
			GroupModel_Stack_Sum += GroupModel_Stack[i];
		}
	}
	FDL_Log("Page = %d, %d, %d",PageList[0],PageList[1],PageList[2]);
	U8 a = 1;
	if (a)
	{
		HostCmdResponse();
