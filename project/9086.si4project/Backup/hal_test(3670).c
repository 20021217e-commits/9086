...
void FA_ReadDisturb(void)
{
	...
	/*
	U16 GroupModel_Stack_Idx = 0;
	U16 *GroupModel_Stack = (U16 *)(MEM_DMA_BUFF_ADDR + 4000);			//累计WL数量数组
	U16 *GroupModel_SP = (U16 *)(MEM_DMA_BUFF_ADDR + 5000);				//起始点数组
	U16 *GroupModel_Len = (U16 *)(MEM_DMA_BUFF_ADDR + 6000);			//页数量数组

	GroupModel_Stack[GroupModel_Stack_Idx] = GroupModel[0];
	GroupModel_SP[GroupModel_Stack_Idx] = 1;
	GroupModel_Len[GroupModel_Stack_Idx] = 0;
	for (i = 1; i < GroupModelLen; i++)
	{
		if (GroupModel[i] == 65535)
		{
			i++;	//额外加一个索引
			GroupModel_Stack_Idx++;
			GroupModel_Stack[GroupModel_Stack_Idx] = GroupModel[i];
			GroupModel_SP[GroupModel_Stack_Idx] = i+1;
			GroupModel_Len[GroupModel_Stack_Idx] = 0;
		}
		else
		{
			GroupModel_Len[GroupModel_Stack_Idx]++;
		}
	}
	//GroupModel_Stack_Idx 同时也代表组数
	*/
	/*
	for (i = 0; i < GroupModel_Stack_Idx; i++)
	{
		FDL_Log(

"GroupModel_Stack=%d",GroupModel_Stack[i]);
		FDL_Log(

"GroupModel_SP=%d",GroupModel_SP[i]);
		FDL_Log(

"GroupModel_Len=%d",GroupModel_Len[i]);
	}
	*/
	/*
	U16 GroupModel_Stack_Sum = 0;
	for (i = 0; i < GroupModel_Stack_Idx; i++)
	{
		if (WlAddr >= GroupModel_Stack_Sum + GroupModel_Stack[i])
		{
			PageList[0] += GroupModel_Stack[i] * GroupModel[GroupModel_SP[i]];
		}
		else
		{
			//FDL_Log("i=%d,WlAddr=%d,GroupModel_Stack_Sum=%d,GroupModel_SP[i]=%d",i,WlAddr,GroupModel_Stack_Sum,GroupModel_SP[i]);
			PageList[0] += (WlAddr - GroupModel_Stack_Sum+1) * GroupModel[GroupModel_SP[i]];
			for (j = 1; j < GroupModel_Len[i]; j++)
			{
				PageList[j] = PageList[0] + GroupModel[GroupModel_SP[i] + j];
			}
			PageCnt = GroupModel_Len[i];
			break;
		}
		GroupModel_Stack_Sum += GroupModel_Stack[i];
	}
	*/
	//__enable_performance();				
	...
	//FDL_Log("Page = %d, %d, %d, %d",PageList[0],PageList[1],PageList[2],PageList[3]);
	/*
	U8 a = 1;
	if (a)
	{
	...
	*/
	//FDL_Log("[0]WLInLastRound=%d",*WLInLastRound);
	SG_DMA2_CH(bChan)->rECC_CTRL.bits.ECC_EN = 0;
	SG_DMA2_CH(bChan)->rENABLE.wAll = 1;
	SG_DMA2_CH(bChan)->rRND_CTRL.bits.CRC_EN=0;
	SG_DMA2_CH(bChan)->rRND_CTRL.bits.RND_BYPASS=1;
	...
	while(1)
	{
		...
		for(j=0;j<4;++j)
		{
			if (GroupModel[WL*4+j] == 65535) break;
...
#endif	
		if (WlAddr == 65535) WL = (WL + 1) % TotalWLCnt;
		dReadTime--;			
		if (dReadTime == 0) break;
	}
	*WLInLastRound = WL;
